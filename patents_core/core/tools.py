...
"""
    ...
    if hasattr(query, 'keywords') and query.keywords:
        ...
        for i, kw in enumerate(query.keywords):
            ...
            # Use BigQuery SEARCH function for keyword matching across multiple fields
            keyword_wheres.append(f"SEARCH(localized_patents, @{param_name})")
            query_params.append(ScalarQueryParameter(param_name, "STRING", kw.lower()))
        if keyword_wheres:
