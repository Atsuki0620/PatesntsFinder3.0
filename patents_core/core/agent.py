...
from patents_core.core.tools import build_patent_query, search_patents_in_bigquery
...
def load_similarity_weights():
    weights_path = Path(__file__).resolve().parents[2] / "config" / "weights.yaml"
    with open(weights_path, 'r', encoding='utf-8') as f:
        config = yaml.safe_load(f)
    return config.get("similarity_weights", {"title": 0.4, "abstract": 0.4, "claims": 0.2})
...
SIMILARITY_WEIGHTS = load_similarity_weights()
...
# 調査観点明確化用プロンプト
PROMPT_CLARIFY_VIEWPOINTS = ChatPromptTemplate.from_template(
    """あなたは、特許調査の専門家です。ユーザーの曖昧な調査依頼を、具体的な調査観点に分解するのを手伝います。
以下のユーザーの初期調査依頼を分析し、調査の方向性を特定するための質問を生成してください。

**ユーザーの初期調査依頼:**
{user_query}

**あなたのタスク:**
1. ユーザーの依頼から、考えられる調査の切り口を複数（3〜5個）提案してください。
2. 各切り口は、以下のような観点を参考にしてください。
    - **[課題]** どのような問題を解決したいのか？ (例: 「製造コストを30%削減する技術」)
    - **[技術特徴]** どのような技術的要素に興味があるのか？ (例: 「特定の化合物Xを用いた触媒技術」)
    - **[応用分野]** どのような分野への応用を想定しているのか？ (例: 「電気自動車向けのバッテリー冷却技術」)
    - **[IPC分類]** 関連しそうな特許分類は何か？ (例: 「G06F 17/00: デジタル計算またはデータ処理」)
3. 提案の最後に、ユーザーにどの観点で調査を進めたいか、または他の観点がないかを問いかけてください。

**質問の例:**
「ご依頼ありがとうございます。調査の精度を高めるため、どのような観点で特許を探しましょうか？ 例えば、以下のような切り口が考えられます。

1.  **[課題]** 〇〇という問題を解決するための技術
2.  **[技術特徴]** △△という特定の技術要素に関するもの
3.  **[応用分野]** □□業界で使われる技術

どの番号に近いですか？ もしくは、他の具体的なイメージがあれば教えてください。」
...
# メインキーワード抽出用プロンプト
PROMPT_SELECT_MAIN_KEYWORDS = ChatPromptTemplate.from_template(
    """あなたは、特許検索の専門家です。与えられた調査方針とキーワードリストを分析し、検索の「主語」となる最も重要な「メインキーワード」を1〜2個選択してください。
...
**あなたのタスク:**
1. 調査方針の中心的な「モノ」や「技術分野」を表すキーワードを1〜2個選びます。これは検索結果全体を縛るための最も重要なキーワードです。
2. 日本語とその英訳がある場合は、両方をリストに含めてください。
3. 選択したメインキーワードを、Pythonのリスト（`List[str]`）形式で出力してください。

**例:**
調査方針: 「ドローンによる配送技術」
キーワードリスト: `["ドローン", "配送", "物流", "自動運転", "drone", "delivery"]`
出力: `["ドローン", "drone"]`

**出力（Pythonのリスト形式のみを出力し、前後に説明や ```python ... ``` などを付けないこと）:**
"""
)
...
PROMPT_SUMMARIZE_PATENTS = ChatPromptTemplate.from_template(
    ...
    以下の複数の特許情報について、**提供された特許リストの情報に厳密に基づき**、調査方針との関連性を分析し、報告書を作成してください。
    ...
    **報告書の構成:**
    1.  **各特許の核心技術:**
        - 提供された各特許情報から、その核心的な技術内容を1〜2文で客観的に要約してください。
        - あなたの推測やリストにない情報は含めないでください。

    2.  **全体サマリー:**
        - 上記の要約全体を俯瞰し、共通する技術的課題や、各社のアプローチの違いを解説してください。
        - このサマリーも、必ず提供された特許リストの情報のみを根拠としてください。

    3.  **調査方針との関連性（最重要）:**
        - **各特許が、上記の「調査方針」にどの程度関連するのかを「高」「中」「低」の3段階で評価してください。**
        - **関連性が「高」または「中」と判断した特許については、その理由を調査方針のキーワードと絡めて具体的に説明してください。**
        - **関連性が「低」と判断した特許については、なぜ関連性が低いのかを簡潔に述べてください。**

    4.  **結論:**
        - **調査方針に対して関連性の高い特許（「高」「中」と評価したもの）のみを基に**、今回の検索結果から導き出される結論を簡潔に述べてください。
    """
)
...
def continue_dialogue(state: AppState) -> AppState:
    ...
    response += "\n\n調査の方向性が固まってきましたら、AIが調査方針を生成します。"
...
def analyze_results(state: AppState) -> AppState:
    ...
    if df is None or df.empty:
        ...
        state.selected_patents_for_summary = [] # 要約対象を空にする
        return state
    ...
    try:
        ...
        # すべての検索結果を要約対象とする
        state.selected_patents_for_summary = state.analyzed_results['publication_number'].tolist()
...
search_execution_workflow.add_node("summarize_selected_patents", summarize_selected_patents) # summarize_selected_patentsノードを追加
...
search_execution_workflow.add_edge("analyze_results", "summarize_selected_patents") # analyze_resultsの後に要約を実行
search_execution_workflow.add_edge("summarize_selected_patents", END) # 要約の後に終了
